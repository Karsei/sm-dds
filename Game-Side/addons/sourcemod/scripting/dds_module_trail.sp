...
#define DDS_ADD_NAME			"Dynamic Dollar Shop :: [Addon] Trail"
...
public void Entity_CreateTrail(int client)
{
	...
	***************************************/
	...
	SelectedStuffToString(sItemEnv, "ENV_DDS_PROPERTY_ADRS", "||", ":", sTrail_ModelSet, sizeof(sTrail_ModelSet));

	// 색상 정보 로드
	char sTrail_ColorSet[32];
	SelectedStuffToString(sItemEnv, "ENV_DDS_PROPERTY_COLOR", "||", ":", sTrail_ColorSet, sizeof(sTrail_ColorSet));

	// 로드했을 때 값이 유효하지 않을 경우 직접 치환
	if (StrEqual(sTrail_ColorSet, "", false))	Format(sTrail_ColorSet, sizeof(sTrail_ColorSet), "0 0 0 0");
