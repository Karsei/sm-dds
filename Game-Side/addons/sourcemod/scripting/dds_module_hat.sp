/************************************************************************
 * Dynamic Dollar Shop - [Module] Hat (Sourcemod)
 * 
...
{
	name = "Dynamic Dollar Shop :: [Module] Hat",
	author = "Eakgnarok",
...
{
	dds_eConvar[HSHOWHATBUGSWITCH] = CreateConVar("dds_switch_show_hat_bug", "1", "forward 값을 가진 모델에 따라 모자를 보여줄지의 여부입니다. 작동을 원하지 않으시다면 0을, 원하신다면 1을 써주세요.", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	
	HookEvent("player_spawn", Event_OnPlayerSpawn);
	HookEvent("player_death", Event_OnPlayerDeath);
	
	DDS_PrintToServer("'Dynamic Dollar Shop :: [Module] Hat' has been loaded.");
}
...
{
	if (DDS_IsPluginOn())
		DDS_CreateGlobalItem(HATID, "모자", "hat");
}
...
{
	// 모자와 날개는 3인칭 시점(콘솔의 thirdperson 는 X)에서만 보이도록 설정
...
*******************************************************/
/* 모자 처리 함수 */
public SetHat(client, const String:model[], bool:create)
{
	if (create)
	{
		if (!FindForwardValue(client, "forward"))
		{
			//DDS_PrintToChat(client, "현재 씌워진 모델이 forward 값을 가지고 있지 않아, 정상적으로 모자를 쓸 수 없습니다.");
			
			if (!GetConVarBool(dds_eConvar[HSHOWHATBUGSWITCH]))
				return;
		}
		
		#if defined _DEBUG_
		DDS_PrintDebugMsg(0, false, "HAT: %d (Set: Create-PREV, client: %d)", dds_iItemEnt[client], client);
		#endif
		...
		// 각도 설정
		DDS_GetItemInfo(DDS_GetUserItemID(client, HATID), 8, tempval);
		...
		// 위치 설정
		DDS_GetItemInfo(DDS_GetUserItemID(client, HATID), 7, tempval);
		ExplodeString(tempval, " ", exstr, 3, 128);
		...
		#if defined _DEBUG_
		DDS_PrintDebugMsg(0, false, "HAT: %d (Set: Create-AFTER, client: %d)", dds_iItemEnt[client], client);
		//DDS_PrintDebugMsg(client, true, "(HAT) 모델: %s, Pos: %f %f %f, Ang: %f %f %f", model, origin[0], origin[1], origin[2], angle[0], angle[1], angle[2]);
		#endif
	}
	else
	{
		#if defined _DEBUG_
		DDS_PrintDebugMsg(0, false, "HAT: %d (Set: NoCreate-PREV, client: %d)", dds_iItemEnt[client], client);
		#endif
		
		if (IsValidEntity(dds_iItemEnt[client]) && (dds_iItemEnt[client] != -1))
		{
			SDKUnhook(dds_iItemEnt[client], SDKHook_SetTransmit, SetTransmit);
			AcceptEntityInput(dds_iItemEnt[client], "Kill");
			
			#if defined _DEBUG_
			DDS_PrintDebugMsg(0, false, "HAT: %d (Set: NoCreate-AFTER, client: %d)", dds_iItemEnt[client], client);
			#endif
		}
		dds_iItemEnt[client] = -1;
	}
...
public Action:Event_OnRoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		...
		{
			new String:hatadrs[128];
			
			DDS_GetItemInfo(DDS_GetUserItemID(i, HATID), 3, hatadrs);
			if (DDS_GetUserItemStatus(i, HATID) && (DDS_GetUserItemID(i, HATID) > 0) && DDS_GetItemUse(HATID))
				SetHat(i, hatadrs, true);
		}
	}
...
public Action:Event_OnPlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (IsClientInGame(client) && IsPlayerAlive(client) && !IsFakeClient(client))
	{
		...
		DDS_GetItemInfo(DDS_GetUserItemID(client, HATID), 3, hatadrs);
		if (DDS_GetUserItemStatus(client, HATID) && (DDS_GetUserItemID(client, HATID) > 0) && DDS_GetItemUse(HATID))
			SetHat(client, hatadrs, true);
	}
...
public Action:Event_OnPlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (DDS_GetUserItemStatus(client, HATID) && (DDS_GetUserItemID(client, HATID) > 0))
		SetHat(client, "", false);
